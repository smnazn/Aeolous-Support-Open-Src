...
module.exports = {
    ...
    async messageRun(message, args) {
        ...
        if (user) {
            ...
            message.reply('Uso: .clearinvites <user>');
        }
    },
    ...
    async handleClear(source, user) {
        ...
        try {
            await Invite.findOneAndUpdate(
                ...
                { $set: { regular: 0, fake: 0, rejoins: 0, leaves: 0, bonus: 0, total: 0 } },
                { upsert: true }
            );
            await reply({ content: `${DISCO_ICONS.CHECKMARK} Se han reseteado las invitaciones de ${user.tag}.`, ephemeral: true });
        } catch (e) {
